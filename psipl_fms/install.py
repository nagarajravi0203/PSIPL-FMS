...
def after_install():
    ...
    frappe.clear_cache()
    print("✅ PSIPL FMS installation setup completed")
...
def create_contact_submission_webform():
    ...
    if not frappe.db.exists("DocType", "Contact Submission"):
        print("❌ DocType 'Contact Submission' does not exist. Please create it first.")
        return
    ...
    if not frappe.db.exists("Web Form", {"route": "contact-submission"}):
        ...
        webform.insert(ignore_permissions=True, ignore_if_duplicate=True)
        frappe.db.commit()
...
def create_lead_entry_webform():
    ...
    if not frappe.db.exists("DocType", "Lead Entry"):
        print("❌ DocType 'Lead Entry' does not exist. Please create it first.")
        return
    ...
    if not frappe.db.exists("Web Form", {"route": "lead-entry"}):
        webform = frappe.get_doc({
            ...
            "doc_type": "Lead Entry",
            "web_form_fields": [
                {"fieldname": "customer_phone_number", "label": "Contact Number", "fieldtype": "Data"},
                {"fieldname": "email", "label": "Email", "fieldtype": "Data"},
                {"fieldname": "lead_type", "label": "Lead Type", "fieldtype": "Select",
                 "options": "Hot Lead\nWarm Lead\nCold Lead"},
                {"fieldname": "company_name", "label": "Company Name", "fieldtype": "Select",
                 "options": "1 BY 2 FOODS PRIVATE LIMITED\nA SQUARE EQUIPMENTS"},
                {"fieldname": "contact_person", "label": "Contact Person", "fieldtype": "Data"},
                {"fieldname": "customer_type", "label": "Customer Type", "fieldtype": "Select",
                 "options": "Old customer - Direct Order (CRR)\nNew customer - Fresh Lead (NBD)\nOld customer - Fresh Lead (NBD CRR)\nP - Old customer - Fresh Lead (P - NBD CRR)"},
                {"fieldname": "segment", "label": "Segment", "fieldtype": "Select",
                 "options": "Kiosk\nMall / Cinemas\nHotel\nRetail\nCafe\nKitchen Equipment Company\nIT / Corporate company\nDealer Enquiry\nSME (small medium enterprise)\nConsultant\nRestaurant\nBakery\nIndustrial Canteen\nTea / coffee shop\nFood Processing Industry\nFood Service Company\nOthers"},
                {"fieldname": "user_email", "label": "User Email", "fieldtype": "Data"},
                {"fieldname": "source_of_lead", "label": "Source of Lead", "fieldtype": "Select",
                 "options": "Phone\nWhats App\nEmail\nIn person\nExhibition\nYou Tube\nIndia Mart\nWeb Site\nInstagram"},
                ...
                {"fieldname": "lead_assigned_to", "label": "Lead Assigned To", "fieldtype": "Select",
                 "options": "Melvin\nGoutham\nRajesh\nRamanathan\nUmesh\nDealer\nKavi Anand\nSC NBD and P-NBD CRR\nDevelopment Team\nKarthik\nSudhir Nayak\nZia\nSc (sharmila)\nYesuraja\nDarshan\nMurali Krishnan\nCRM (NBD CRR)\nSpare CRM\nSc NBD CRR\nTamilNadu\nAP and Telangana\nMaharashtra\nKarnataka"},
                {"fieldname": "dealer_name", "label": "Dealer Name", "fieldtype": "Select",
                 "options": "CROCKERY WORLD\nHOTEL WORLD"},
                {"fieldname": "need_analysis", "label": "Need Analysis", "fieldtype": "Select",
                 "options": "Required\nNot Required"},
                {"fieldname": "category", "label": "Category", "fieldtype": "Select",
                 "options": "I Brew\nKitchen Equipment\nPots and pans\nI Brew + Kitchen Equipment\nAll the above\nDelibo\nSpares"},
                {"fieldname": "have_you_met_the_client", "label": "Have you met the client?", "fieldtype": "Select",
                 "options": "Voice Call\nVideo Call\nIn Person\nNo"},
                {"fieldname": "is_demo_done", "label": "Is Demo Done?", "fieldtype": "Select",
                 "options": "Yes\nNo"},
                {"fieldname": "next_step", "label": "Next Step", "fieldtype": "Select",
                 "options": "Need to Meet\nRequired Quotation\nGive Demo\nUpdate in OTD\nRequired PI\nInvalid lead\nAssign to Dealer\nFuture Prospective Lead"},
                {"fieldname": "price_details", "label": "Price Details", "fieldtype": "Select",
                 "options": "Maximum Selling Price\nRe-seller Price\nList Price\nDealer Price\nNot Applicable\nService provider price"},
